import json


class Who:
    def __init__(self, name, age):
        self.name = name
        self.age = age


class Who1:
    def __init__(self, name, age):
        self.name = name
        self.age = age


class MyEncoder(json.JSONEncoder):
    def default(self, e):
        print("In redefined default method. Object type:", type(e))
        if isinstance(e, Who):
            print("Serialization possible.")
            return e.__dict__
        else:
            print("Serialization is not possible.")
            return (
                "Object type is not supported."  # 1st - returning of serializable info.
            )
            # return super().default(e)  # 2nd - returning of defualt encoding (to raise a TypeError).


some_man = Who("John Doe", 42)
other_man = Who1("Roman Opala", 60)

python_objects_to_encoding = [
    "a",
    2,
    3.0,
    [1, 2, 3],
    {"a": 1, "b": 3, "c": 3},
    True,
    None,
    some_man,
    {"name": "Piotr Opala", "age": 29},
    other_man,
]

encoded_python_objects = []
for o in python_objects_to_encoding:
    json_object = json.dumps(o, cls=MyEncoder)
    encoded_python_objects.append(json_object)

# How .dumps method works:
# Checking all parameters.
#   If defaults - run json.JSONEncoder().encode() encoder.
#   If not - checking if cls parameter (encoder) was redefined.
#     If was - initiate cls() encoder and run .encode() method with defined parameters.
#       Method uses default function to encode other objects.
#         If none - run built-in function what is simple raising error.
#         If not (redefined) - run redefined function.
#     If was not - initiate json.JSONEncoder() encoder and run .encode() method with defined parameters.
#       Method uses default function to encode other objects.
#         If none - run built-in function what is simple raising error.
#         If not (redefined) - run redefined function.
